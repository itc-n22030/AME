...
Slackボットを使って出席管理を行い、"出席"メッセージを送信すると情報を記録し、状況に合わせたメッセージをbotへ出力させ、定期的にリマインダーメッセージを送信します。

#### ・sheet.js

Google SheetsやSlackの設定を管理し、データベースから設定更新や、Google Sheetsとの連携を行い、出席情報の取得や更新を行う。
...
学生がログインし、自身のシートを確認する機能を行い、ユーザーがログインすると、データベースで認証され、その学生のシート情報がGoogle Sheets APIを介して取得されます。
